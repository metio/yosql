...
The package name for all utility classes.
...
```console
$ yosql --jdbc-utility-package-name=configValue
```

As long as the name of the config option is unique across all configuration groups, you can use the shorter form:

```console
$ yosql --utility-package-name=configValue
```

### Gradle

In order to use `YoSQL` together with [Gradle](https://gradle.org/), take a look at the tooling [documentation for Gradle](/tooling/gradle/). The `utilityPackageName` setting can be configured using Gradle in Kotlin syntax like this:
...
yosql {
  jdbc {
    utilityPackageName.set(configValue)
  }
}
```
...
yosql {
  jdbc {
    utilityPackageName = configValue
  }
}
```
...
In order to use `YoSQL` together with [Maven](https://maven.apache.org/), take a look at the tooling [documentation for Maven](/tooling/maven/). The `utilityPackageName` setting can be configured using Maven like this:
...
<build>
  <plugins>
    <plugin>
      ...
      <configuration>
        <jdbc>
          <utilityPackageName>configValue</utilityPackageName>
        </jdbc>
      </configuration>
    </plugin>
  </plugins>
