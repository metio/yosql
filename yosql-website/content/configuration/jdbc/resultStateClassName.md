...
The class name of the result-state class
...
```console
$ yosql --jdbc-result-state-class-name=configValue
```

As long as the name of the config option is unique across all configuration groups, you can use the shorter form:

```console
$ yosql --result-state-class-name=configValue
```

### Gradle

In order to use `YoSQL` together with [Gradle](https://gradle.org/), take a look at the tooling [documentation for Gradle](/tooling/gradle/). The `resultStateClassName` setting can be configured using Gradle in Kotlin syntax like this:
...
yosql {
  jdbc {
    resultStateClassName.set(configValue)
  }
}
```
...
yosql {
  jdbc {
    resultStateClassName = configValue
  }
}
```
...
In order to use `YoSQL` together with [Maven](https://maven.apache.org/), take a look at the tooling [documentation for Maven](/tooling/maven/). The `resultStateClassName` setting can be configured using Maven like this:
...
<build>
  <plugins>
    <plugin>
      ...
      <configuration>
        <jdbc>
          <resultStateClassName>configValue</resultStateClassName>
        </jdbc>
      </configuration>
    </plugin>
  </plugins>
