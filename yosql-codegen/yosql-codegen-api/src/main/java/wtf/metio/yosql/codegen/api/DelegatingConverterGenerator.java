 ...
 */
public final class DelegatingConverterGenerator implements ConverterGenerator {

    private final ApiConfiguration apis;
    private final Set<ConverterGenerator> generators;

    /**
     * @param apis       The API configuration.
     * @param generators The generators to delegate to.
     */
    public DelegatingConverterGenerator(
            final ApiConfiguration apis,
            final Set<ConverterGenerator> generators) {
        this.apis = apis;
    ...
    public Stream<PackagedTypeSpec> generateConverterClasses(final List<SqlStatement> statements) {
        return dao().generateConverterClasses(statements);
    }

    private ConverterGenerator dao() {
